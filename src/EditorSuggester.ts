...
export class EditorSuggester extends EditorSuggest<VerseLink> {
  ...
  async selectSuggestion(value: Verse, evt: MouseEvent | KeyboardEvent): Promise<void> {
    if (this.context) {
      const editor = this.context.editor as Editor;
      editor.replaceRange(await value.toReplace(), this.context.start, this.context.end);
